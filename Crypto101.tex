...
may not be able to simply get the secret from eavesdropping, an \emph{active}
attacker can still break the system. If that attacker (Mallory) is in
between Alice and Bob, they can perform the Diffie-Hellman
...
%**HMM** ?? Alice and Bob do NOT have a shared secret, i.e., there is no single secret number they both know. There is a shared secret between Alice and Mallory, and /another/ shared secret between Mallory and Bob. Therefore, the consequence "The attacker can then simply take all the messages they get from one person and send them to the other" is also wrong or at least quite misleading, isn't it? It must be:
Alice and Bob will believe to have a shared secret, but actually, there is
one secret shared between Alice and Mallory, and \emph{another} one between Bob and
Mallory. The attacker can then simply take all the messages they get
from one person, decrypt and re-encrypt them and send the result to the other.
Between decrypting and encrypting, they can look at the
plaintext messages, they can modify them, and they can even remove messages
or insert new ones---all without Alice or Bob seeing anything other than
messages correctly encrypted with their respective secret.
