...
followed by something the attacker chooses. However, it turns out that for many
...
Prefix MACs, i.e., the construction $H(k \| m)$ for a MAC, 
is actually secure in current (SHA-3-era) hash
...
We'll illustrate that our example MAC is insecure if it is used to authenticate
...
using a cryptographic hash function. Anyone can then use the public key to
...
which is the opposite of how it encryption and decryption work.
# **HMM** encryption and decryption are not "usual(ly)" in contrast to signing, I'd say.
...
TODO: explain parameter generation and "per user keys" (see #51)
# **HMM** ..., says Wikipedia - that's where the x and y come from which I didn't finde defined in the following formulas.
...
\]
#+END_LATEX
# **HMM** I can only compare this with Wikipedia - there, it says that r = (g^k \pmod p) \pmod q
...
being executed for or the like. \cite{rfc5869}
...
is that, like so many cryptographic systems gone wrong, to the
untrained eye, getting it wrong looks exactly like getting it right.
# **HMM** That was a syntactically "abusive" sentence - only after I put the commas right, I found that there was one "that" too much :)
...
# **HMM** I'd suggest that the level-2 headers follow the three items in introduction - therefore I changed some header levels from here on.
*** Cryptographically secure pseudorandom number generators

# **HMM** Explain why Cryptographically secure pseudorandom number generators are different from "normal" ones; probably it suffices to point to the section on the Mersenne Twister.
...
*** A standard pseudorandom number generator: Mersenne Twister
...
$2^{19937} - 1 \approx 4 \cdot 10^{6001}$, and it passes all but the most demanding randomness
